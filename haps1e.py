  ...
  with gzip.open(infile, 'rb') as fp:
    ...
    while line :
      ...
      if pre_filter_strict_pass(cts):
        Gloc.add_edge(loc1, loc2, order=[loc1, loc2], conf=True, cts=cts, mns=mns, sds=sds, wt=1.0/tot)
      elif pre_filter_loose_pass(cts):
        Gloc.add_edge(loc1, loc2,  order=[loc1, loc2], conf=False, cts=cts, mns=mns, sds=sds, wt=1.0/tot)
      line=fp.readline().strip().decode()
      ct+=1
