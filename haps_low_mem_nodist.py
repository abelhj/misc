...
def main():
      ...
      if len(pos)>2:
        for ii in range(len(pos)-1):
          for jj in range(1, len(pos)):
            ...
            add_edge(locus[ii], locus[jj], allele[ii], allele[jj],  Gloc)
      line=fp.readline().strip()
      linect+=1
  ...
  with open(args.debugfile, 'w') as outd:
    for edge in edgelist:
      ...
      print(edge[0]+'\t'+edge[1]+'\t'+ctstr,  file=outd)
...
def add_edge(loc1, loc2, all1, all2, gg):
  if not gg.has_edge(loc1, loc2):
    gg.add_edge(loc1, loc2)
    curedge=gg.edges[loc1, loc2]
    curedge.update({'l1': loc1, 'l2': loc2, 'counts':[0,0,0,0]})
    if curedge['l1']==loc1:
