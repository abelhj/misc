...
def main():
    ...
    while line:
      ...
      for r1 in reads[read].keys():
        if r1!=allele:
          ...
          Gall.edges[edge0]['reads'].append(read)
  ...
  for edge in edgelist:
    ...
    if (aa+bb==0 and cc*dd>0) or (cc+dd==0 and aa*bb>0) or (aa+cc==0 and bb*dd>0) or (bb+dd==0 and aa*cc>0):
      ...
      remove_allele_edges(Gloc, Gallconf, edge)
  ...
  with open(args.edgefile, 'w') as outf, open(args.hapfile, 'w') as outh:
    ...
    for ii in range(len(comp_loose)):
      ...
      for jj in range(len(gg)):
        ggsub1=gg[jj]
        haps=phase_conf_component(ggsub1)
        for hapid in range(2):
          Gallsub=Gallconf.subgraph(haps[hapid]).copy()
          ...
          for treeid in range(len(mintree)):
            for edge in mintree[treeid].edges():
              print(str(ii)+'_'+str(jj)+'_'+str(hapid+1)+'_'+str(treeid)+'\t'+str(edge[0])+'\t'+str(edge[1]), file=outh)
            terminal_nodes=[]
            ...
            for aa in range(len(terminal_nodes)-1):
              for bb in range(aa+1, len(terminal_nodes)):
                ...
                for node1 in sp:
                  print(str(ii)+'_'+str(jj)+'_'+str(hapid+1)+'_'+str(treeid)+'\t'+str(aa)+'_'+str(bb)+'\t'+str(mintree[treeid].degree(node1))+'\t'+node1, file=outf)
