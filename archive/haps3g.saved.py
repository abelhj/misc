...
def main():
  ...
  loc2all=defaultdict(dict)
  all2loc={}
  with gzip.open(infile, 'rb') as fp:
    line=fp.readline().strip()
    while line and readct<3000:
      [allelestr, readstr, readpos, strand]=re.split('[\t]', line)
      [locstr, ref, alt, refalt]=re.split('_', allelestr)
      readpos=int(readpos)
      ...
      if not locstr in locid:
        ...
        locct+=1
      locus=locid[locstr]
      allr=locstr+'_'+ref+'_'+alt+'_r'
      alla=locstr+'_'+ref+'_'+alt+'_a'
      allid[alla]=allct
      id2all[allct]=alla
      loc2all[locus]['a']=allct
      all2loc[allct]=locus
      allct+=1
      allid[allr]=allct
      id2all[allct]=allr
      loc2all[locus]['r']=allct
      all2loc[allct]=locus
      allct+=1
      allele=allid[allelestr]
...
#  code.interact(local=locals())
  ...
  for edge in Gall.edges:
    ...
    add_counts_edge(Gloc, Gall, edge[0], edge[1], curedge['count'], all2loc)
  print('locus graph')
  print('Memory usage info (Mb):\t'+str(resource.getrusage(resource.RUSAGE_SELF).ru_maxrss/usage_denom))
  ...
  for edge in edgelist:
    ...
    if (aa+bb==0 and cc*dd>0) or (cc+dd==0 and aa*bb>0) or (aa+cc==0 and bb*dd>0) or (bb+dd==0 and aa*cc>0): # could add addl paralog filter here
      remove_allele_edges(edge[0], edge[1], Gall, loc2all)
  ...
  with open(args.edgefile, 'w') as outf, open(args.hapfile, 'w') as outh:
    comp_loose=list(Gloc.subgraph(c).copy() for c in sorted(nx.connected_components(Gloc), key=len, reverse=True))
    ...
    for ii in range(len(comp_loose)):
      ...
      for jj in range(len(gg)):
        ggsub1=gg[jj]
        print(str(ii)+'\t'+str(jj))
        [h1, h2]=phase_conf_component(ggsub1, loc2all, id2loc)
        ...
        for hapid in range(2):
          ...
          for treeid in range(len(mintree)):
            ...
            for tredge in treelist:
              ...
              print(str(ii)+'_'+str(jj)+'_'+str(hapid+1)+'_'+str(treeid)+'\t'+edgestr+'\t'+id2all[tredge[0]]+'\t'+id2all[tredge[1]], file=outf)
            terminal_nodes=[]
            ...
            for aa in range(len(terminal_nodes)-1):
              for bb in range(aa+1, len(terminal_nodes)):
                ...
                for nodeii in range(len(sp)):
                  ...
                  print(str(ii)+'_'+str(jj)+'_'+str(hapid+1)+'_'+str(treeid)+'\t'+str(aa)+'_'+str(bb)+'\t'+outstr+'\t'+id2all[node1], file=outh)        
...
def add_counts_edge(gg, ggall, node1, node2, count, all2loc):
  [locus1, locus2]=[all2loc[node1], all2loc[node2]]
...
def remove_allele_edges(node1, node2, ggall, loc2all):
  for aa in loc2all[node1].values():
    for bb in loc2all[node2].values():
      if ggall.has_edge(aa, bb):
        ggall.remove_edge(aa, bb) 


def phase_conf_component(ggsub, loc2all, id2loc,  flip=False, strict=True):
  ...
  [h1, h2]=phase_from_node(ggsub, maxnode, phased_nodes, bad_nodes, loc2all, strict)
  return([h1, h2])

def get_phased_allele(gg, node, hap, loc2all):
  refalt=gg.nodes[node]['phased_all'][hap]
  return(loc2all[node][refalt])


def phase_from_node(gg1, startnode, phased_nodes, bad_nodes, loc2all, strict=True):
  hap1=[]
  hap2=[]
  if len(gg1)==1:
    hap1.append(get_phased_allele(gg1, startnode, 1, loc2all))
    hap2.append(get_phased_allele(gg1, startnode, 2, loc2all))
  else:
    edgelist=list(nx.bfs_edges(gg1, startnode))
    edge0=edgelist[0]
    curnode=edge0[0]
    hap1.append(get_phased_allele(gg1, curnode, 1, loc2all))
    hap2.append(get_phased_allele(gg1, curnode, 2, loc2all))
    for ii in range(len(edgelist)):
      ...
      if 'phased_all' in gg1.nodes[curnode]:
        ...
        if not strict or (strict and  (aa*dd>0 or bb*cc>0)): #see both haps
          ...
          hap1.append(get_phased_allele(gg1, nextnode, 1, loc2all))
          hap2.append(get_phased_allele(gg1, nextnode, 2, loc2all))
      else:
